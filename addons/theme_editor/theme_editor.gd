...
func set_stylebox_color(stylebox_name: String, theme_type: String, stylebox: StyleBox, color: Color) -> void:
  if stylebox is StyleBoxFlat:
    stylebox.bg_color = color
    if stylebox.get_border_width_min() > 0:
      var border_radius: int = (get_stylebox("normal", theme_type) as StyleBoxFlat).corner_radius_bottom_left
